...
import { Clock, Users, ChefHat, Bookmark, Heart, Share2, Edit, Loader2, ShoppingBag, Check, Trash2, Wand2,
// Add missing food-related icons
Beef, Fish, Apple, Egg, Wheat, Carrot, Utensils } from "lucide-react";
...
const RecipeDetail = () => {
  ...
  return <MainLayout title={recipe.title} showBackButton={true} action={<div className="flex gap-2">
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              ...
              <AlertDialogFooter>
                ...
                <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={handleDeleteRecipe} disabled={isDeleting}>
                  {isDeleting ? <>
        ...
        <div className="page-container my-0 mx-[5px] px-[28px] py-[17px]">
          <div className="flex items-center justify-between mb-6">
            <div className="flex flex-wrap gap-4">
              ...
              {recipe.servings && <div className="flex items-center text-muted-foreground">
                  <Users className="w-4 h-4 mr-1" />
                  <span className="text-sm">{recipe.servings} servings</span>
                </div>}
              {recipe.difficulty && <div className="flex items-center text-muted-foreground">
            ...
            <div className="flex gap-2">
              <Button variant="ghost" size={isMobile ? "sm" : "icon"} onClick={() => setIsFavorited(!isFavorited)} className={isMobile ? "p-1" : ""}>
                <Heart className={`h-5 w-5 text-primary ${isFavorited ? 'fill-primary' : ''}`} />
              </Button>
              <Button variant="ghost" size={isMobile ? "sm" : "icon"} className={isMobile ? "p-1" : ""}>
                <Bookmark className="h-5 w-5 text-primary" />
              </Button>
              <Button variant="ghost" size={isMobile ? "sm" : "icon"} onClick={handleOpenShareDialog} className={isMobile ? "p-1" : ""}>
          ...
          <div className="mb-6">
            <AiSuggestionButton onClick={handleOpenVariationsDialog} label="Create Variations" variant="cheese" isLoading={parsingMealSuggestion} className="w-full md:w-auto">
              ...
              <ScrollArea maxHeight="350px">
                <ul className="space-y-3">
                  ...
                  const {
                    mainText,
                    preparation
                  } = parsePreparation(cleanedIngredient);
                  const {
                    name,
                    amount
                  } = parseIngredientAmount(mainText);
                  return <li key={index} className="flex items-center p-2 bg-sage-50 rounded-md border border-sage-200 hover:bg-sage-100 transition-colors">
                        ...
                        <div className="flex-1">
                          <span className="text-sm">
                            ...
                            {(prepInstructions || preparation) && <span className="text-xs text-muted-foreground ml-1">
          ...
          {recipe.tags && recipe.tags.length > 0 && <div className="mb-6">
              <h3 className="font-medium mb-2">Tags</h3>
              <div className="flex flex-wrap gap-2">
                {recipe.tags.map(tag => <span key={tag} className={`px-3 py-1 rounded-full text-sm border-2 border-black font-medium ${getPastelColorForTag(tag)}`}>
                    {tag}
                  </span>)}
              </div>
            </div>}
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <Button className="w-full" onClick={() => navigate("/meal-plan")}>
              Add to Meal Plan
            </Button>
          </div>
        </div>
      ...
      <SuggestMealDialog open={suggestDialogOpen} onOpenChange={setSuggestDialogOpen} currentDay={null} currentMealType={null} suggestMealType={suggestMealType} setSuggestMealType={setSuggestMealType} aiLoading={aiLoading} suggestedMeal={suggestedMeal} parsingMealSuggestion={parsingMealSuggestion} additionalPreferences={additionalPreferences} setAdditionalPreferences={setAdditionalPreferences} onSuggestMeal={handleSuggestMeal} onSaveSuggestedRecipe={handleSaveSuggestedRecipe} onResetSuggestedMeal={handleResetSuggestedMeal} />

      <RecipeVariationsDialog open={variationsDialogOpen} onOpenChange={setVariationsDialogOpen} recipeName={recipe.title} onGenerateVariation={handleGenerateVariation} isLoading={parsingMealSuggestion} />

      <ShareRecipeDialog open={shareDialogOpen} onOpenChange={setShareDialogOpen} recipeName={recipe.title} onShare={handleShareRecipe} />
